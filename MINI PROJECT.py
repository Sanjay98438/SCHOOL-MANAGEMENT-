    ...
    def stafflog(self):
        while True:
            ...
            if ch == "1":
                ...
                sfpas = input("Enter Your Password")
                l.append(self.sfid)
                l.append(sfname)
                l.append(sfpas)
                self.stflo[self.sfid] = l
                print("Your staff Id Is:", self.sfid)
                self.sfid += 1
            ...
            if ch == "1":
                ...
                try:
                    if aid == self.studet[aid][0]:
                        self.studet[aid][3] = True
                        self.studet[aid][6] = "Approved"
                ...
                for i in self.studet:
                    print(
                        "%-10s     %-11s     %-14s      %-5d     %-7s" % (
                        i, self.studet[i][1], self.studet[i][6], self.studet[i][5], self.studet[i][7]))
                    print("-----------------------------------------------------------------------")
                    with open("studentsdetails.txt", 'a') as f:
                        print(
                            "%-10s     %-11s     %-14s      %-5d     %-7s" % (
                            i, self.studet[i][1], self.studet[i][6], self.studet[i][5], self.studet[i][7]), file=f)
                print("=======================================================================")
            elif ch == "5":
                break
